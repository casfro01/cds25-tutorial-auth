...
public class DraftController(IDraftService service) : ControllerBase
{
    ...
    public async Task<long> Create(ClaimsPrincipal claims, DraftFormData data) => await service.Create(claims, data);
    ...
    [Route("{id}")]
    [Authorize(Roles = Role.Editor)]
    public async Task Update(long id, DraftFormData data) => await service.Update(HttpContext.User, id, data);
